...
class DWAPlanner:
    ...
    def __init__(self, config=None, config_manager=None):
        ...
        if config_manager:
            ...
            # DWA参数 - 这些将作为基础配置
            default_config = {
                'max_speed': 10.0,         # 最大线速度 (m/s)
                'min_speed': 0.0,         # 最小线速度 (m/s) (可以是负数，如果允许倒退)
                'max_omega': 5.0,         # 最大角速度 (rad/s)
                'min_omega': -5.0,        # 最小角速度 (rad/s)
                'max_accel': 1.5,         # 最大线加速度 (m/s^2)
                ...
                'robot_radius': 0.5,           # 机器人等效半径(m)，用于碰撞检测
    ...
    def calc_obstacle_cost(self, trajectory, obstacles, current_safe_distance, dt):
        ...
        robot_radius = self.base_config.get('robot_radius', 0.0) # 获取机器人半径
            ...
            for obs in obstacles:
                ...
                if isinstance(obs, dict):
                    ...
                    if obs_geometry.get('type') == 'circle':
                        ...
                        if dist_sq <= (obs_r + robot_radius)**2:
                            return float('inf') # 发生碰撞
                        
                        dist_to_surface = math.sqrt(dist_sq) - obs_r - robot_radius
...
class SimulationVisualizer:
    ...
    def __init__(self, planner):
        ...
        self.scale = 2 # 1米 = 2像素 (原为20，代表40x30米)
        self.robot_length = 70.0 # 船体长度 (m)
        self.robot_width = 3.0   # 船体宽度 (m)

        self.canvas = tk.Canvas(self.window, width=self.canvas_width, height=self.canvas_height, bg="white")
        self.canvas.pack()

        # 初始化模拟状态 - 适应更大的400x300米场景
        self.current_pos = [30.0, 30.0, np.pi / 4] # 从左下角出发
        self.current_vel = 0.0
        self.current_omega = 0.0
        self.goal = [350.0, 250.0] # 目标在右上角
        self.obstacles = [
            # --- "地狱级"难度场景 ---
            # 动态障碍物 (5个, 修改为10x5米的矩形)
            {"id": 101, "type": "vessel", "position": {"x": 150.0, "y": 280.0}, "velocity": {"vx": 0.0, "vy": -10.0}, "geometry": {"type": "rectangle", "length": 10.0, "width": 5.0}},
            {"id": 102, "type": "vessel", "position": {"x": 380.0, "y": 180.0}, "velocity": {"vx": -1.2, "vy": 1.2}, "geometry": {"type": "rectangle", "length": 20.0, "width": 10.0}},
            {"id": 103, "type": "vessel", "position": {"x": 80.0, "y": 120.0}, "velocity": {"vx": 1.0, "vy": 0.5}, "geometry": {"type": "rectangle", "length": 30.0, "width": 10.0}},
            {"id": 104, "type": "vessel", "position": {"x": 300.0, "y": 20.0}, "velocity": {"vx": -5.5, "vy": 1.0}, "geometry": {"type": "rectangle", "length": 40.0, "width": 25.0}},
            {"id": 105, "type": "vessel", "position": {"x": 350.0, "y": 250.0}, "velocity": {"vx": -1.5, "vy": -5.2}, "geometry": {"type": "rectangle", "length": 50.0, "width": 25.0}},
            # 静态障碍物 (3个, 使用统一的geometry格式)
            {"id": 201, "type": "structure", "position": {"x": 120.0, "y": 200.0}, "geometry": {"type": "circle", "radius": 20.0}},
            {"id": 202, "type": "structure", "position": {"x": 250.0, "y": 100.0}, "geometry": {"type": "circle", "radius": 25.0}},
            {"id": 203, "type": "structure", "position": {"x": 200.0, "y": 150.0}, "geometry": {"type": "circle", "radius": 22.0}}
        ]
...
if __name__ == '__main__':
    ...
    test_config = {
        'max_speed': 10.0, 'min_speed': 0.0, 'max_omega': 5.0, 'min_omega': -5.0,
        'max_accel': 1.5, 'max_domega': 1.0, 'v_resolution': 0.1, 'omega_resolution': 0.1,
        'dt': 0.1, 'predict_time': 5.0, # 对于大船和高速，需要更长的预测时间
        'base_heading_weight': 0.6, 'base_dist_weight': 0.25, 
        'base_velocity_weight': 0.6, # 鼓励机器人移动而不是停止
        'base_obstacle_weight': 1.7, 
        'base_safe_distance': 20.0, # 对于大船，需要更大的安全距离
        'robot_radius': 35.0, # 船体等效半径 (m)，设置为70米船长的一半
    }
